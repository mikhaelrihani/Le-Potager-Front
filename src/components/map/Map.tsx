...
const Map: React.FC = () => {
  ...
  const fetchDataFromDatabase = async (searchQuery: string) => {
    try {
      const response = await axios.get(
        `https://www.back.potager.mikabernik.com/api/gardens/search?city=${searchQuery}&dist=20`
      );
  ...
  const fetchPictures = async (locationId) => {
    try {
      const response = await axios.get(
        `https://www.back.potager.mikabernik.com/api/gardens/${locationId}/pictures`
      );
      return response.data;
    } catch (error) {
      console.error(
